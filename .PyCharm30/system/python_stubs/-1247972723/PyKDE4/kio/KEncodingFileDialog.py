# encoding: utf-8
# module PyKDE4.kio
# from /usr/lib/python2.7/dist-packages/PyKDE4/kio.so
# by generator 1.135
# no doc

# imports
import PyKDE4.kdeui as __PyKDE4_kdeui
import PyQt4.QtCore as __PyQt4_QtCore
import PyQt4.QtGui as __PyQt4_QtGui


from KFileDialog import KFileDialog

class KEncodingFileDialog(KFileDialog):
    # no doc
    def getOpenFileNameAndEncoding(self, *args, **kwargs): # real signature unknown
        pass

    def getOpenFileNamesAndEncoding(self, *args, **kwargs): # real signature unknown
        pass

    def getOpenUrlAndEncoding(self, *args, **kwargs): # real signature unknown
        pass

    def getOpenUrlsAndEncoding(self, *args, **kwargs): # real signature unknown
        pass

    def getSaveFileNameAndEncoding(self, *args, **kwargs): # real signature unknown
        pass

    def getSaveUrlAndEncoding(self, *args, **kwargs): # real signature unknown
        pass

    def selectedEncoding(self, *args, **kwargs): # real signature unknown
        pass

    def __init__(self, *args, **kwargs): # real signature unknown
        pass

    Result = None # (!) real value is ''


