# encoding: utf-8
# module PyKDE4.kdeui
# from /usr/lib/python3/dist-packages/PyKDE4/kdeui.cpython-34m-x86_64-linux-gnu.so
# by generator 1.135
# no doc

# imports
import PyKDE4.kdecore as __PyKDE4_kdecore
import PyQt4.QtCore as __PyQt4_QtCore
import PyQt4.QtGui as __PyQt4_QtGui
import PyQt4.QtSvg as __PyQt4_QtSvg


class KPageView(__PyQt4_QtGui.QWidget):
    # no doc
    def createView(self, *args, **kwargs): # real signature unknown
        pass

    def currentPage(self, *args, **kwargs): # real signature unknown
        pass

    def currentPageChanged(self, *args, **kwargs): # real signature unknown
        pass

    def faceType(self, *args, **kwargs): # real signature unknown
        pass

    def itemDelegate(self, *args, **kwargs): # real signature unknown
        pass

    def model(self, *args, **kwargs): # real signature unknown
        pass

    def setCurrentPage(self, *args, **kwargs): # real signature unknown
        pass

    def setDefaultWidget(self, *args, **kwargs): # real signature unknown
        pass

    def setFaceType(self, *args, **kwargs): # real signature unknown
        pass

    def setItemDelegate(self, *args, **kwargs): # real signature unknown
        pass

    def setModel(self, *args, **kwargs): # real signature unknown
        pass

    def showPageHeader(self, *args, **kwargs): # real signature unknown
        pass

    def viewPosition(self, *args, **kwargs): # real signature unknown
        pass

    def __init__(self, *args, **kwargs): # real signature unknown
        pass

    Auto = 0
    FaceType = None # (!) real value is ''
    List = 2
    Plain = 1
    Tabbed = 4
    Tree = 3


